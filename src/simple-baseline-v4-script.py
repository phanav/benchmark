...
datadir = "/mnt/beegfs/home/vu/Codalab-MetaDL/data/plantvillage/plantvillage-formatted-image"
...
resultprefix = "mini_plantvillage-finetune"
...
def build_data(imagesize, train_metadata, valid_metadata, imagedir, train_transform, test_transform):
#     imagesize = (128, 128)
#     transform0 = partial(transform_image, imagesize=(imagesize))

    train_dataset = ImageDataset(train_metadata, imagedir, train_transform)
    train_dataloader = get_dataloader(train_dataset, batchsize=128, ifshuffle=True)

    valid_dataset = ImageDataset(valid_metadata, imagedir, test_transform)
    valid_dataloader = get_dataloader(valid_dataset, batchsize=128, ifshuffle=False)
...
#         sum_correct += correct_count
    ...
    return sum_loss/sample_count, score
...
#     optim_param = [dict(params=group, lr=lr, weight_decay=0.1) for group, lr in zip(paramgroups.values(), lrgroup)]
    optim_param = [dict(params=group, lr=lr, weight_decay=10) for group, lr in zip(paramgroups.values(), lrgroup)]
...
#     optim = torch.optim.SGD(model.parameters(), lr=lr, weight_decay=1e-3, momentum=0)
#     optim = torch.optim.SGD(model.parameters(), lr=lr, weight_decay=1e-3, momentum=0.9)
    optim = torch.optim.Adam(model.parameters(), lr=lr, weight_decay=1e-1)
...
modelname = 'resnet18_sgd_dropout0101_weightdecay10'
...
# optim = get_optimizer(model, lr=1e-3)
# optim = get_lrrange_optimizer(model, lrrange=(1e-6, 1e-3), get_optim_param_dict)
optim = get_lrrange_optimizer(model, (1e-7, 1e-3), get_optim_param_dict_resnet)
